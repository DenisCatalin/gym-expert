...
export async function addToFavouritesQuery(token, issuer, gif, name, category) {
  const operationsDoc = `
  mutation addToFavourites($issuer: String!, $gif: String!, $name: String!, $category: String!) {
    insert_favourites(objects: {issuer: $issuer, gif: $gif, name: $name, category: $category}) {
      affected_rows
    }
  }
`;
...
export async function getFavouritesByFilterQuery(token, issuer, filter) {
  const operationsDoc = `
  query getFavourites($issuer: String!, $filter: String!) {
    favourites(where: {
      issuer: {_eq: $issuer},
      category: {_eq: $filter}
    }) {
...
`;
  const response = await queryHasuraGQL(operationsDoc, "getFavourites", { issuer, filter }, token);
