...
`;
  ...
  console.log("TEST2", issuer, email, publicAddress, dateString, token, response);
...
`;
  ...
  console.log("[getUserDetailsQuery] ", issuer, token, response);
  return response;
}

export async function addPurchaseQuery(token, issuer, planName, planPrice, paymentId, email, date) {
  const operationsDoc = `
  mutation addPurchase($issuer: String!, $planName: String!, $planPrice: Float!, $paymentId: String!, $email: String!, $date: String!) {
    insert_purchases(objects: {issuer: $issuer, planName: $planName, planPrice: $planPrice, paymentId: $paymentId, email: $email, date: $date}) {
      affected_rows
    }
  }
`;
...
export async function addTestimonialQuery(token, displayName, date, text, rating, issuer) {
  const operationsDoc = `
  mutation addTestimonial($displayName: String!, $date: String!, $text: String!, $rating: Float!, $issuer: String!) {
    insert_testimonials(objects: {name: $displayName, date: $date, text: $text, rating: $rating, issuer: $issuer}) {
      affected_rows
    }
  }
`;
...
export async function addToFavouritesQuery(token, issuer, gif, name) {
  const operationsDoc = `
  mutation addToFavourites($issuer: String!, $gif: String!, $name: String!) {
    insert_favourites(objects: {issuer: $issuer, gif: $gif, name: $name}) {
      affected_rows
    }
  }
`;
...
export async function checkFavouriteQuery(token, issuer, gif) {
  const operationsDoc = `
    query checkFavourite($issuer: String!, $gif: String!) {
      favourites(where: {issuer: {_eq: $issuer}, gif: {_eq: $gif}}) {
        id
      }
    }
`;
  const response = await queryHasuraGQL(operationsDoc, "checkFavourite", { issuer, gif }, token);

  return response?.data?.favourites?.length;
}

export async function getFavouritesQuery(token, issuer) {
  const operationsDoc = `
  query getFavourites($issuer: String!) {
    favourites(where: {issuer: {_eq: $issuer}}) {
      id
      gif
      name
    }
...
`;
  const response = await queryHasuraGQL(operationsDoc, "getFavourites", { issuer }, token);

  return response;
}

export async function deleteFavouritesQuery(token, issuer, gif) {
  const operationsDoc = `
  mutation deleteFavourites($issuer: String!, $gif: String!) {
    delete_favourites(where: {issuer: {_eq: $issuer}, gif: {_eq: $gif}}) {
      affected_rows
    }
  }
`;
  const response = await queryHasuraGQL(operationsDoc, "deleteFavourites", { issuer, gif }, token);
